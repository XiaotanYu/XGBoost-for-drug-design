...
ML_method = 'Xgboost'
abbreviate = 'xgb'
dev_data = './data/final_analysis_CDK_0828.xlsx'
# val_data   = './excel/val.xlsx'


class_names = ['活性分子', '非活性分子']
...
# 预测
# y_train_pred = classifier.predict(X_one)
...
M = pd.read_excel('./output/Accuracy.xlsx')
sns.boxplot(data=M, x='Model', y='Accuracy', color='#00b8e5')
plt.xlabel('Model', fontsize=30)
plt.ylabel('Accuracy', fontsize=30)
plt.xticks(fontsize=30)
...
#
#
# classifier = make_pipeline(StandardScaler(), RandomForestClassifier(random_state=random_state))
# classifier.fit(X_train, y_train)
#
